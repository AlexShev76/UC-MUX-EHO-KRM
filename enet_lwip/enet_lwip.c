...
int i;
    for(i=0; i< size; i+=4 ){
        if( i % 16 == 0 )
            ...
            logo_printf(" %08X", *((unsigned int*)&Buf[i]));
...
int i, j;
    for(i=0; i< size; i+=4 ){
        if( i % 16 == 0 )
            logo_printf("\n");
        for(j=0;j<32; j++)
            logo_printf("%x", BIT( *((uint32_t*)(&Buf[i])), j) );
        logo_printf(" ");
    }
    logo_printf("\n");
}
...
//  int i;
    ...
    logo_printf( "Init  Peripheral :SYSCTL_PERIPH_ETH \n\r");
...
/**      IP Addres   ip addres    */
    ...
    do{
        ...
        logo_printf( "Enet-lwip Read ID : %d  [%d]\n\r",  SlotID, HARD_IP_adr);
    ...
    if((ulUser0 == 0xffffffff) || (ulUser1 == 0xffffffff))
    {
        ...
        //
        logo_printf("MAC Address Not Programmed! \n");
        while(1);
    }
    ...
    logo_printf("MAC: %02X-%02X-%02X-%02X-%02X-%02X \n",
    ...
    //
    logo_printf("Waiting for IP \r");
    while( !IPSetOk );  logo_printf("Set IP   --- OK\n");
    ...
    logo_printf( "Init Ext IRQ " );     StartInitIRQ( );     logo_printf( "-------- ok \r\n" );
    ...
    if(g_bFirmwareUpdate){
        ...
        if(ulIPAddr == 0) {
            ...
            if(ulIPAddr){
                logo_printf("Locator IP: %d.%d.%d.%d",
...
void DisplayIntStatus(void)
{
    ...
    logo_printf("\rActive: %c%c%c ", (ulTemp & 1) ? '1' : ' ', (ulTemp & 2) ? '2' : ' ', (ulTemp & 4) ? '3' : ' ');

    //
    // Display the currently pending interrupts.
    //
    ulTemp = HWREG(NVIC_PEND0);
    logo_printf("Pending: %c%c%c", (ulTemp & 1) ? '1' : ' ', (ulTemp & 2) ? '2' : ' ', (ulTemp & 4) ? '3' : ' ');
}
...
int i;
    logo_printf("Ku : %d db * 10\r\n", takt->Ku10 );
    logo_printf("VRCH_Count :         %d \r\n", takt->VRCH_Count );

    for(i=0; i<takt->VRCH_Count; i++) logo_printf("%6d |", i ); logo_printf("\r\n");
    for(i=0; i<takt->VRCH_Count; i++) logo_printf("%6d |", takt->VRCH_Time_ns[i] );   logo_printf("\r\n");
    for(i=0; i<takt->VRCH_Count; i++) logo_printf("%6d |", takt->VRCH_Amp_db10[i] );   logo_printf("\r\n");
...
/**  Setup the A-Scan  */
    logo_printf("AScan begin : %d ns  %d\r\n", takt->ABegin_ns, REG_ns_to_ppt(takt->ABegin_ns) );
    logo_printf("AScan width : %d ns \r\n", takt->AWidth_ns );
    ...
    logo_printf("Ku AScan width : %d ns \r\n", AScan_Ku_begin );
